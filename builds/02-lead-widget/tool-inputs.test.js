...
test('у каждого поля инструмента есть осмысленное описание', () => {
  // Пустое описание в $fromAI — не «нейтрально». Проверено дважды на живой
  // странице: пустое описание у address дало выдуманную «Самару», а пустое
  // описание у extension дало допрос — «нужен ли вынос?» до того, как модель
  // вообще узнала конфигурацию. Посетитель пришёл за ценой, а попал в анкету.
  for (const [field, expr] of Object.entries(inputs.value)) {
    if (!expr.includes('$fromAI')) continue;
    const m = expr.match(/\$fromAI\('[a-z_]+',\s*`([^`]*)`/);
    assert.ok(m, `${field}: не разобрал вызов $fromAI`);
    assert.ok(m[1].trim().length > 25,
      `${field}: описание пустое или слишком короткое — модель будет додумывать`);
  }
});

test('необязательные поля прямо запрещают модели о них спрашивать', () => {
  // Этаж и вынос уточняют цифру, но расчёт работает без них. Вопрос о них
  // ДО первой цены — это ровно та анкета, против которой построена сборка.
  for (const field of ['floor', 'extension']) {
    assert.match(inputs.value[field], /НЕ СПРАШИВАЙ/,
